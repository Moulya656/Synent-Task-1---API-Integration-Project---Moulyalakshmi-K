...
searchBtn.addEventListener("click", () => {
    ...
    if (!city) {
        errorText.textContent = "Please enter city name";
        return;
    }
    if (lat && lon) {
...
async function getWeather(latitude, longitude, city) {
    ...
    try {
        ...
        errorText.textContent = "Could not fetch weather";
    }
...
cityInput.addEventListener("input", () => {
    ...
    timer = setTimeout(async () => {
        ...
        try {
            const response = await fetch(
                `https://geocoding-api.open-meteo.com/v1/search?name=${query}&count=50&language=en`
            );
            const data = await response.json();
            if (!data.results) return;
            data.results.forEach((city) => {
                // Only show Indian cities
